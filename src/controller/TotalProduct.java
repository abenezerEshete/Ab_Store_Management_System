...
public class TotalProduct {
    ...
    String                searchquery      = "";
    int                   pos;
    List<DespenceProduct> despenceProductList;
    @FXML
    ...
    private void loadData(int position) {
        ...
        try {
            ...
            // get row count
            Query<StoreProduct> storeCountQuery = Main.getSession().createQuery("FROM  StoreProduct " + formatedSearchQuery);

            int totalRows = storeCountQuery.list().size();
            totalPage = totalRows / pageLimit;
            if (totalRows % pageLimit != 0)
                totalPage++;
            itemPage.setText(" " + currentPage + "/" + totalPage);

            countList.setText("All Product (" + totalRows + ")");
            // get list of tasks
            pos--;
            ...
            Query<DespenceProduct> despenceProductQuery = Main.getSession().createQuery(
                    "from DespenceProduct " + formatedSearchQuery + " order by updateDateTime desc ");
            despenceProductQuery.setFirstResult(startlimit);
            despenceProductQuery.setMaxResults(pageLimit);
            storeProductList = storeProductQuery.getResultList();
            despenceProductList = despenceProductQuery.getResultList();
            List<StoreProduct> totalProduct = FXCollections.observableArrayList();
            storeProductList.forEach(product -> {
                int proIndex = storeProductList.indexOf(product);
                for (int i = 0; i < despenceProductList.size(); i++) {
                    if (despenceProductList.get(i).getProductType() != null) {
                        if (despenceProductList.get(i).getProductType().getId() == product.getProductType().getId())
                            product.setAmount(product.getAmount() + despenceProductList.get(i).getAmount());
                    }
                }
                totalProduct.add(product);
            });
            productTypeTable.getItems().clear();
            productTypeTable.getItems().addAll(totalProduct);
